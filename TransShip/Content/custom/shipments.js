...
model.shipmentsController = {
    ...
    shipmentsLastHisotry: ko.observableArray([]),
    shipmentIndex: 0,
    ...
    mapShipment: function (shipment) {
        ...
        eShipment.purcharse_order(shipment.purcharse_order);
    },
    ...
    showNewShipment: function () {
        ...
        var shipments = model.shipmentsController.shipments();
        var customerId = customerId = model.usersController.user.customerId();
        TransShip.Scripts.jsCheckShipmentsByMembership(customerId, function (data) {
            console.log("CheckShipmentsByMembership");
            console.log(data);
    ...
    save: function () {
        ...
        if (shipmentParam.shipmentId != "")
        {
            model.shipmentsController.update();
            return;
        }
        ...
        //get shipments by po
        model.shipmentsController.getShipmentsByPO(shipmentParam.customerId, shipmentParam.purcharse_order, function (data) {
            if (data !== null)
            {
                if (data.purcharse_order === shipmentParam.purcharse_order) {
                    bootbox.confirm("¿PO# already exists, are you sure to continue?", function (result) {
                        if (result) {
                            //call api save
    ...
    getShipmentsByPO: function (customerId, PO, callback)
    {
        if (PO !== null || PO !== undefined) {
            TransShip.Scripts.jsGetShipmentsByPO(customerId, PO, function (data) {
                console.log("done");
    ...
    isValidStepTwo: function() {
        ...
        var validPO = model.validateForm('#shipForm');

        if (!valid || !validPO) {
            toastr.error("Please complete step 2");

            if (!validPO) { valid = validPO }
        }
        return valid; 
    },
    ...
    step: function(toStep) {
        ...
        if (toStep >= 3) {
            var customerId = model.shipmentsController.shipment.customerId();
            var stateText = model.shipmentsController.shipment.stateText();
            var state = model.shipmentsController.shipment.state();

            if (stateText != "" || stateText != null || stateText != undefined) {
                state = stateText;
            }

            TransShip.Scripts.jsGetLastTwoShipment(customerId, state, function (data) {
                model.shipmentsController.shipmentsLastHisotry(data);
                
            });

            //automatic save the shipment on the step three to up only if no save before
            var shipmentId = model.shipmentsController.shipment.shipmentId();
            if (typeof shipmentId === "undefined" || shipmentId === "" || shipmentId === "0") {
                model.shipmentsController.save();
            }
        }
